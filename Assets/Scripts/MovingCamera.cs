...
public class MovingCamera : MonoBehaviour
{
    ...
    [SerializeField]
    private float smooth_step = 10f;
    [SerializeField]
    private float smooot_weight = 0.4f;
    [SerializeField]
    ...
    public void LookAround()
    {
        ...
        look_angles.x += current_mouse_look.x * sensibility * (invert ? 1f : -1f);
        look_angles.y += current_mouse_look.y * sensibility;

        look_angles.x = Mathf.Clamp(look_angles.x, default_look_limits.x, default_look_limits.y);
        current_roll_angle = Mathf.Lerp(current_roll_angle, Input.GetAxis("Mouse X") * roll_angle, Time.deltaTime * roll_speed);
